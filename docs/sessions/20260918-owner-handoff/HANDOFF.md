...
```
listener bound to 127.0.0.1:63902 (IPv4 only)
  connect to 127.0.0.1 : REACHED
  connect to ::1       : FAILED (ConnectionRefusedError)   <-- what 'localhost' tries FIRST
  browser-order (localhost -> ::1): FAILED (ConnectionRefusedError)
```
...
- **The on-disk Tauri `target/` directory is corrupt, and plain `cargo check` there fails until it is reset.** Reproduced twice on 2026-09-18: `cd crates/fabric-gui/src-tauri && cargo check` dies with
  ...
  **This is not a code defect.** The same crate compiles cleanly with a fresh target directory
  (`CARGO_TARGET_DIR=<fresh> cargo check` → `Finished dev profile in 4m 03s`), and `/Applications/Phenotype Fabric.app` was built from this source on 2026-09-17. The `target/` tree was left half-generated, most likely by an interrupted build.
  **Fix:** point `CARGO_TARGET_DIR` somewhere fresh, or delete `crates/fabric-gui/src-tauri/target/`. Do not "fix" it in source.
  **Why it matters beyond the inconvenience:** because `src-tauri` is excluded from the workspace, no CI job ever compiles this crate except `release.yml`'s `build-gui`, which is gated on a `v*` tag that has never been pushed. So a corrupt or unbuildable state here is invisible to every automated gate.
